...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const pageUrl = `${baseUrl}/${locale}/birth-pieces`;
  ...
  return {
    ...
    alternates: {
      canonical: pageUrl,
      languages: {
        nl: `${baseUrl}/nl/birth-pieces`,
        en: `${baseUrl}/en/birth-pieces`,
      },
    },
    openGraph: {
      title: t("title"),
...
const BirthPiecesPage = async ({ params }: Props) => {
  const { locale } = await params;
  const p = await getTranslations({ locale, namespace: "paragraphs.birth" });
...
export default BirthPiecesPage;
